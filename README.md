...
AI-assisted triage tool for Wazuh SIEM alerts, using a locally hosted LLM (Ollama) so security data stays on-machine. Built on real alerts generated by the Wazuh Blue Team Detection Lab rather than synthetic sample data.

- Python preprocessing pipeline for real Wazuh alerts (JSON/JSONL), with a parser handling multiple Wazuh rule structures
- Local LLM integration via Ollama
- In progress: prompt engineering for alert triage and automated incident summary generation
- Planned: RAG with a FAISS vector store, MITRE ATT&CK technique mapping, exportable investigation reports

**Stack:** Python, Wazuh SIEM, Ollama, Docker, JSON/JSONL *(planned: FAISS, LangChain)*
