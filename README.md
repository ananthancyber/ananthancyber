...
- Python parser handling multiple Wazuh alert JSON/JSONL structures, with MITRE ATT&CK technique suggestion constrained to an explicit "insufficient evidence" fallback when unsupported
- Streamlit dashboard: upload alerts, trigger analysis, view and download structured Markdown investigation reports
- Automated pytest suite covering source-IP extraction, prompt construction, and report generation
  
**Stack:** Python, Wazuh SIEM alert format, Ollama, FAISS, Streamlit, pytest
